...
def array_sum_with_index(arr)
    if arr.length > 0
    arr.map!.with_index { |num, idx| num * idx }
        return arr.reduce(:+)
    else
        return 0
    end
...
def num_factors(number)
    factors = []
    (1..number).each do |num| if number % num == 0 
        factors << num 
        end
    end
...
def ordered_vowel_word?(word)
    ...
    word.each_char do |char|
      ...
      if vowels.length != chars.length 
        vowels.pop
      elsif vowels.length == chars.length && vowels == chars
        return true
      else
        return false
      end
...
def products_except_me(numbers)
    arr = []
    numbers.each do |ele| arr << array_product(numbers)/ele
    end
    return arr
end
